...
# - 数据类型检测或类型比较：
#     - type(value)	检查数据属于哪种类型
#         - 不会认为子类是一种父类类型，不考虑继承关系。
#     - isinstance(value, (dict, int, str等))	检测参数的是否符合指定的类型集合，返回布尔值
#         - 会认为子类是一种父类类型，考虑继承关系。（推荐使用）
#     
# 
# ## 函数方法：
#     def 方法名()	例：
...
# - random			伪随机数生成器
# - json         JSON (JavaScript Object Notation) 是一种轻量级的数据交换格式。它基于ECMAScript的一个子集。
