...
- ```go test xxx_test.go``` 编译测试程序
- ```go test -test.bench=.*``` 运行所有的基准测试函数，其中基准测试函数必须以Benchmark开头
...
3. number 数字类型
   ...
   3. float 浮点型
      ...
      2. float64 +- 5 1e-324 -> 107 1e308 精确到小数点后 15 位，推荐优先使用，也是小数点数值的默认类型
...
24. new() 用于值类型， make() 用于引用类型（不包括interface）
...
31. 不要使用打印状态来表明通道的发送和接收顺序：由于打印状态和通道实际发生读写的时间延迟会导致和真实发生的顺序不同。
32. Go 语言中，没有引用传递，函数都是按照值传递，即 passed by value。如果函数调用时，参数是一个指针，则 Go 会把指针进行隐式转换得到 value，但返回来则不行。
33. Windows 系统换行用"\r\n", Linux 平台下使用 "\n"
34. <font color="red">defer 仅在函数返回时才会执行,在循环的结尾或其他一些有限范围的代码内不会执行</font>
35. <font color="red">当切片作为参数传递时,切记不要解引用切片。</font>解引用后，传递的就是拷贝后的值了
36. <font color="red">永远不要使用一个指针指向一个接口类型，因为它已经是一个指针。</font>
...
48. 类型与方法，必须在同一个包中被声明，这也是为什么不能在int、float或类似这些的类型上定义方法的原因。（但是可以使用内嵌结构体或者基于其它包的类型定义新类型，来规避此规则）
49. 只有接口变量才可以使用类型断言
50. type-switch 进行运行时类型判断时，不允许有fallthrough
51. select 对 case 的选择是伪随机的
...
- 当有多个 defer 注册时，执行顺序为逆序执行（类似于栈，先进后出）
- 内置函数：
  ...
  5. make 分配内存，用于内置引用类型（切片slice、字典(map)、管道channel），不包括interface
...
- 类型检测: v.(type) 此种方式只能在 switch 中使用，且需提前知道有几种类型，函数没有返回值
- interface{} 类型的变量类型断言，如 ```str := val.(string)```
...
- 结构体字段命名冲突时，外层名字会覆盖内层名字，同一级别出现两次，使用时会引发错误。
- Go 语言中的继承是通过内嵌或组合来实现的
- Go 语言中，一个类型加上它的方法等价于面向对象中的一个类
- 类型关联的方法不能写在类型结构里面；类型和方法的关联由接收者来建立
- 类型与方法，必须在同一个包中被声明，非本地类型不能定义方法（但是可以使用内嵌结构体或者基于其它包的类型定义新类型，来规避此规则）
- 使用type定义新类型，只继承类型的属性，不继承类型的方法，因为方法是跟类型绑定的。
- 同一个包中的内嵌结构体，属性和方法都会继承。非一个包的内嵌结构体，只会继承对外提供的方法。
- 结构体实例可以通过字段名直接访问任意一级的属性或方法
- 内嵌结构体的字段名就是它的类型名

- 函数和方法的区别（定义方式不同、调用方式不同）：
  1. 函数将变量作为参数：fn1(recv); 方法在变量上被调用：recv.Method1()
  2. 当接收者或参数为指针时，方法和参数都可以改变其值或状态
  3. 方法中，接收者必须有一个显式的名字，且必须在方法中被使用

- 方法也是可以通过内嵌结构体，实现继承，或者在功能类型中声明一个具名字段
...
- 类型变量赋值接口变量时，必须满足接口变量的全部方法，才能赋值，否则编译时将报错。
- ```var _ Inface = (*Struct1)(nil)```（不会分配内存）或者```var _ Iface = new(Struct1)```可以利用此方法的报错，判断一个类型是否实现了某个接口。
...
- 以上规则总结为，在接口上调用方法时：
  1. 指针方法只能通过指针调用，不能通过值调用，因为存储在接口中的值没有地址
  2. 值方法可以通过值调用或指针调用，因为指针会自动解引用

- 只要接口变量实现了其他接口的所有方法，就可以动态转换，调用方法。
- 实现了某个接口的类型可以被传给任何以此接口为参数的函数。
- 每个 interface{}变量在内存中占据两个字节，一个用来存储包含它的类型，一个用来存储它包含的数据或者指向数据的指针
- 赋值数据切片至空接口切片，必须使用 for-range 语句一个一个地复制，不能直接赋值，否则将报错。
...
- 反射包 reflect
  ...
  - ```func (reflect.Value).CanSet() bool``` 报告v的值是否可以被改变，只有当值是可寻址的并且是导出的结构字段，值才能被更改
  - ```func (reflect.Value).Elem() reflect.Value```  返回接口v所包含的值或指针v所指向的值，如果v的类型不是interface或Ptr，会报错
  - ```func (reflect.Value).SetFloat(x float64)``` 设置v的值，还有其他类型v.SetBool()、v.SetBool()...等等
  - ```func (reflect.Type).Field(i int) reflect.StructField``` 返回结构体v的第i个字段，不是结构体或超出范围，会报错
  - ```func (reflect.Type).NumField() int``` 返回结构内的字段数量
  - ```func (reflect.Value).Method(i int) reflect.Value``` 返回第一个方法
  - ```func (reflect.Value).FieldByName(name string) reflect.Value``` 返回带有给定名称的结构字段。如果没有找到字段，则返回零值。如果v的类不是struct，它会惊慌。
  - ```func (reflect.Value).Index(i int) reflect.Value``` 返回v的第i个元素。如果v的类型不是Array、Slice或String，或者i超出了范围，它就会恐慌。
...
- panic 会导致栈被展开，直到 defer 修饰的 recover()被调用或者程序中止
...
- 如果需要更多面向对象的能力，参考[goop 包（Go Object-Oriented Programming）](https://github.com/lanl/goop)


# 未能深刻理解的点：
reflect、interface、文件读写、类型转换
